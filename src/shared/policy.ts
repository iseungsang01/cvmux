...
export const POLICY = {
   ...
   *
   * "지금 셸이 돌고 있다"를 60초 늦게 알려주는 것은 알려주지 않는 것과 같다.
   * 실측 근거: powershell 기동 347ms + WMI 전체 열거 265~900ms(부하에 따라).
   * 전부 워커에서 도므로 타이핑에는 닿지 않는다(P14-12). 유휴 상태에서는
   * 위의 60초로 돌아간다.
   */
  PROCESS_TREE_BUSY_AGE_MS: 10_000,
