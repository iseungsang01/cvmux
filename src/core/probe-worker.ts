 ...
 *
 * 그 멈춤이 데몬의 메인 루프에서 일어나면 PTY 입출력도 함께 멎는다 — 사용자가
 * 친 키가 그 동안 셸에 닿지 못하고, 루프가 풀리는 순간 밀린 입력이 한꺼번에
 * 쏟아진다. "한 자리에서 타이핑되다 갑자기 밀려 나오는" 증상의 정체가 이것이다.
 *
 * 그래서 조사는 전부 여기서 돈다. 막히는 것은 이 워커의 루프뿐이고, 데몬은
