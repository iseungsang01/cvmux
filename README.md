...
- **세션이 앱보다 오래 산다** — PTY를 별도 데몬이 들고 있어,
  앱을 강제 종료해도 돌던 작업은 계속된다
- **에이전트 알림** — Claude Code 같은 CLI가 확인을 기다리면 파란 링과 토스트로 알려준다
- 분할 창 · 세션 영속성 · 트레이 상주 · 한글 UTF-8 · 트루컬러
...
PTY는 앱이 아니라 **별도 데몬 프로세스**가 들고 있다. 창을 닫아도, 앱을 완전히
종료해도, 심지어 작업 관리자로 강제 종료해도 **돌던 작업은 계속된다**. 앱은 그
세션을 들여다보는 창일 뿐이라 언제든 다시 붙을 수 있다. 실측으로 확인했다 —
앱을 강제 종료한 12초 동안 세션의 작업이 13줄 더 진행됐고, 앱을 다시 열자 같은
세션이 '실행 중' 상태로 그대로 돌아왔다.
...
저장된 세션의 자리를 잡아 둔다. 창은 뜨지 않는다. 다만 **전원을 껐다 켜면
프로세스는 남지 않는다** — 돌아오는 것은 작업 디렉토리와 화면이지 실행 중이던
명령이 아니다. 그 경계를 흐리지 않는다.
