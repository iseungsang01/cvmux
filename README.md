...
분할 키는 Windows Terminal과 같다. 분할 경계는 드래그로 조정할 수 있고, 각 칸은
최소 120px까지만 줄어든다.
...
- **세션 데몬** — PTY는 앱이 아니라 **별도 프로세스**가 들고 있다. 창을 닫아도,
  앱을 완전히 종료해도, 심지어 작업 관리자로 강제 종료해도 **돌던 작업은 계속된다**.
  앱은 그 세션을 들여다보는 창일 뿐이라 언제든 다시 붙을 수 있다.
  실측으로 확인했다 — 앱을 강제 종료한 12초 동안 세션의 작업이 13줄 더 진행됐고,
  앱을 다시 열자 같은 세션이 '실행 중' 상태로 그대로 돌아왔다

- **트레이 상주** — 창을 닫으면 트레이로 물러난다. 끝내는 방법은 두 갈래다:
  '창 닫기'는 앱만 닫고 세션을 남기며, '세션까지 모두 종료'는 전부 정리한다.
  잠깐 창을 치우려던 사람이 돌던 빌드를 날리는 일이 없도록 갈라 두었다

- **로그인할 때 미리 준비** — 트레이 메뉴에서 켜면 로그인 직후 데몬이 조용히 떠서
  저장된 세션의 자리를 잡아 둔다. 창은 뜨지 않는다.
  다만 **전원을 껐다 켜면 프로세스는 남지 않는다** — 돌아오는 것은 작업 디렉토리와
  화면이지 실행 중이던 명령이 아니다. 그 경계를 흐리지 않는다

- **셸 통합** — 세션이 시작할 때 프로필의 프롬프트를 감싸 작업 디렉토리(OSC 7)와
  프롬프트 경계(OSC 133)를 심는다. `cd`로 옮겨 다녀도 사이드바의 저장소와 git 정보가
  따라오고, 상태 판정도 휴리스틱 대신 셸이 직접 알려주는 신호를 쓴다. 프로필 파일은
  건드리지 않는다 — 이 세션 안에서만 유효하다 (`CVMUX_NO_SHELL_INTEGRATION=1`로 끌 수 있다)

- **세션 영속성** — 앱을 껐다 켜면 작업 디렉토리·제목·스크롤백이 돌아온다.
  복원된 내용 뒤에는 `── 이전 세션 (복원됨) ──` 구분선을 그어, 그게 죽은 텍스트이고
  새 셸이 그 자리에 섰다는 사실을 감추지 않는다

- **분할 창** — 사이드바 한 줄이 워크스페이스이고 그 안에 pane을 여럿 둘 수 있다.
...
```
src/
  core/                Electron에 기대지 않는 알맹이 — 데몬이 쓴다
    ansi-parser.ts     증분 ANSI/OSC 파서 (청크 경계에 걸린 시퀀스 처리)
    session-state.ts   상태 판정 엔진 (P4)
    pty-manager.ts     PTY 생명주기, 프로세스 트리 정리, 출력 배칭
    protocol.ts        앱 ↔ 데몬 계약 (프레이밍·요청·이벤트)
    store.ts           세션·배치 영속성 (P16/P17)
  daemon/              세션을 들고 있는 프로세스 (P20)
    index.ts           Named Pipe 서버. ELECTRON_RUN_AS_NODE로 돈다
  main/                Electron 메인 프로세스 — 뷰어 쪽
    daemon-client.ts   데몬 연결·기동·프로토콜 협상
    ipc.ts             렌더러 ↔ 데몬 중계
    tray.ts            트레이 상주 (P18)
...
앱과 데몬은 서로 다른 프로세스이므로 `core/protocol.ts`가 유일한 접점이다.
데몬은 Electron API를 쓸 수 없고, 써서도 안 된다 — 토스트나 대화상자처럼
사람에게 보이는 것은 전부 앱의 몫이다.
