...
- 에이전트가 답을 `[완료]`로 시작하면 넘기지 않고 사람에게 알린다 — 할 일이 끝났다는 표시
- 사람 입력 없이 10번 이어지면 스스로 꺼진다. 한 번 끼어들면 다시 센다. 횟수는 **⇄를 우클릭**해 바꾼다
- 켜 둔 연결은 앱을 다시 켜도 남는다

**Codex에게 줄 설명서** — [docs/codex.md](./docs/codex.md). Codex에게 "이 설명서대로 설치해
줘"라고 하면 스스로 훅을 걸고 협업 규칙을 `~/.codex/AGENTS.md`에 넣는다(`/hooks` 승인과
재시작은 사람 몫).
