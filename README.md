...
| ◌ 작은 점선 원 | 꺼져 있음 | 복원만 해 두고 셸을 아직 띄우지 않았다 — 누르면 켜진다 |
...
**세션 영속성** — 앱을 껐다 켜면 워크스페이스 배치·작업 디렉토리·제목이 돌아온다.
이전 화면(스크롤백)은 되살리지 않는다 — 새 셸이 빈 화면으로 그 자리에 선다.

**복원한 세션은 볼 때 켠다.** 앱을 켜면 지난번 세션이 전부 사이드바에 돌아오지만,
셸은 **처음 보이는 것 하나**만 뜬다. 나머지는 "꺼져 있음 · 누르면 켭니다"로 남아
메모리를 전혀 쓰지 않다가, 사이드바에서 누르는 순간 PowerShell이 뜬다(에이전트
이어서 띄우기도 그때 돈다). 셸 하나가 conhost까지 80MB 남짓이라, 열어 둔 세션이
많을수록 차이가 크다.

늘 돌고 있어야 하는 줄은 사이드바에서 줄에 마우스를 올리고 **📌**를 누른다
(팔레트의 "앱을 켤 때 함께 켜기"도 같다). 고정한 줄은 앱이 뜨자마자 켜지고,
고정하는 순간에도 켜진다. 한 번 켠 세션은 다시 끄지 않는다 — 보이지 않는다고
멈추면 뒤에서 돌던 작업이 죽는다.
